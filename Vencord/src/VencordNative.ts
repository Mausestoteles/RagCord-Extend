...
export default {
    ...
    ragcord: {
        /**
         * Returns a sanitised view of the current RagnaMod session — never
         * includes the raw token. Null if no session is stored.
         */
        getSession: () => invoke<RagCordSessionUser | null>(IpcEvents.RAGCORD_GET_SESSION),
        /** Calls /api/logout server-side and clears the local session file. */
        logout: () => invoke<boolean>(IpcEvents.RAGCORD_LOGOUT),
        /**
         * Fetches the forum news list via main-process auth. Returns null
         * if there is no session or the request failed for any reason.
         */
        fetchNews: () => invoke<RagCordNewsItem[] | null>(IpcEvents.RAGCORD_FETCH_NEWS),
        /**
         * Verknüpft die übergebene Discord-User-ID mit dem aktuellen RagnaMod-
         * Account. Backend speichert in `discord_links`; der RagnaMod-Discord-
         * Bot liest das beim nächsten Member-Sync und vergibt die Verified-
         * Rolle + setzt den Nicknamen auf den MC-Namen.
         */
        linkDiscord: (discordId: string, discordName?: string | null) =>
            invoke<{ ok: boolean; error?: string; }>(
                IpcEvents.RAGCORD_LINK_DISCORD, discordId, discordName ?? null),
        /**
         * Forum-Benachrichtigungen vom Backend. Null bei No-Session oder
         * Netzfehler — Renderer behandelt das wie "Skip dieser Tick".
         */
        fetchNotifications: () =>
            invoke<{ count: number; items: RagCordNotification[]; } | null>(
                IpcEvents.RAGCORD_FETCH_NOTIFICATIONS),
    },

    pluginHelpers: PluginHelpers
};

export interface RagCordSessionUser {
    member_id: number;
    username: string;
    name: string | null;
    mcname: string | null;
...
export interface RagCordNewsItem {
    id: number;
    title: string;
    body: string;
    author: string;
...
export interface RagCordNotification {
    id: number;
    type?: string | null;
