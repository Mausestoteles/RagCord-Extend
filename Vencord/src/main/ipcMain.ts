...
import { writeFileAtomicSync } from "./utils/safeWriteFile";

// QuickCSS is user-edited CSS that we re-inject on every change. Cap it at a
// generous-but-finite size so a buggy or malicious plugin can't fill the disk
// or the file watcher can't OOM the renderer with a giant string push.
const MAX_QUICK_CSS_BYTES = 5 * 1024 * 1024;
...
ipcMain.handle(IpcEvents.SET_QUICK_CSS, (_, css) => {
    if (typeof css !== "string") {
        throw new Error("SET_QUICK_CSS: expected a string");
    }
    if (Buffer.byteLength(css, "utf-8") > MAX_QUICK_CSS_BYTES) {
        throw new Error(`SET_QUICK_CSS: payload exceeds ${MAX_QUICK_CSS_BYTES} bytes`);
    }
    writeFileAtomicSync(QUICK_CSS_PATH, css);
});
