...
import { runLoginGate } from "./ragcord/loginGate";
import { migrateFromUpstreamIfNeeded } from "./ragcord/migrateUpstream";
import { RendererSettings } from "./settings";
import { IS_VANILLA } from "./utils/constants";

console.log("[RagCord] Starting up...");
...
if (!IS_VANILLA) {
    ...
    // One-shot: if upgrading from upstream Vencord, pull the old data dir
    // into the new RagCord location so settings/themes/QuickCSS survive the
    // switch. Skipped if RagCord is already initialised or no Vencord dir
    // exists. Safe to call every launch.
    migrateFromUpstreamIfNeeded();

    // RagCord login gate: every consumer that awaits app.whenReady() — which
    // includes Discord's main-window creation — is delayed until the
    // RagnaMod auth flow completes. If a previously stored session token
    // verifies the gate resolves immediately; otherwise we open the login
    // window and only resolve once it succeeds. Closing the window without
    // logging in calls app.quit() inside runLoginGate.
    const origWhenReady = app.whenReady.bind(app);
    let gatedReady: Promise<void> | null = null;
    app.whenReady = () => {
        gatedReady ??= origWhenReady().then(runLoginGate);
        return gatedReady;
    };
    ...
    console.log("[RagCord] Running in vanilla mode. Not loading RagCord Extend");
}

console.log("[RagCord] Loading original Discord app.asar");
